"""
Tests for vanilla Transformer model adapter.

Standard Transformer encoder–decoder baseline for time series tasks.
"""
...
class TestTransformerForecast:
    ...
    def test_adapter_instantiation(self, forecast_config):
        ...
        model = TransformerAdapter(forecast_config)
    ...
    def test_gelu_activation(self, forecast_config, sample_forecast_inputs):
        from liulian.models.torch.transformer import TransformerAdapter

        config = {**forecast_config, 'activation': 'gelu'}
        model = TransformerAdapter(config)
        outputs = model.run(sample_forecast_inputs)
        validate_forecast_output(outputs, 4, 24, 7)


class TestTransformerOtherTasks:
    @pytest.fixture
