...
- **Experiment configs live under `experiments/<experiment>/configs/`** (user decision 2026-08-07: configs follow the EXPERIMENT directory, not the dataset directory). Example: the hydro-LLM entry `experiments/hydro_llm/run_matrix.py` reads `experiments/hydro_llm/configs/{timellm_config,tier0_ettcontrol,debug}.yaml`. Do not scatter configs across dataset folders; when adding a new experiment, create its `configs/` subdir alongside its runner. This includes the HPO search grid: the `search_space_file` config key points at a per-experiment `search_spaces.yaml` (same schema as the default); omitted -> `liulian/optim/search_spaces.yaml`, set-but-missing -> loud error.
