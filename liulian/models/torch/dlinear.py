...
DLinear uses simple linear layers on decomposed trend and seasonal components.
Despite its simplicity, it often outperforms complex transformer-based models.
"""
...
class DLinearAdapter(TorchModelAdapter):
    ...
    def _prepare_model_inputs(self, inputs: Dict[str, torch.Tensor]) -> tuple:
        ...
        Expected inputs:
            ...
            - x_dec: [batch, label_len + pred_len, dec_in] - decoder input (optional)
            - x_mark_dec: [batch, label_len + pred_len, mark_dim] - decoder time features (optional)
        """
        x_enc = inputs['x_enc']
