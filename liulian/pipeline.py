...
def build_dataset(config: Dict[str, Any]) -> Any:
    ...
    if data_name.startswith('swiss-river') or data_name == 'camels-chem':
...
def _load_prompt_content(config: Dict[str, Any]) -> str:
    ...
    * ``none``               -> ``''``            (the model skips the prompt prefix entirely)
    ...
    prompt_map = {
        ...
        'camels-chem': 'wt-swiss-1990',  # same BAFU network; reuse the swiss domain prompt
    }
...
def build_model(config: Dict[str, Any], dataset: Any = None) -> Any:
    ...
    _INTERNAL_IDENTITY_MODELS = frozenset({'timellm', 'gpt4ts', 'tempo', 'autotimes', 'calf'})
...
def run_experiment(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Provenance (2026-08-24, 10-way-audit issue E): dump the FULL resolved config
    # into the artifact dir. spec.yaml serializes only a schema subset — it omits
    # llm_random_init / llm_layers / llm_tuning / llm_model_id / prompt_richness /
    # holdout_stations / the resolved embedding_size, so backbone/tuning arms were
    # only distinguishable via transient job logs. resolved_config.yaml makes every
    # artifact self-certifying (matches the entity_identifier harness convention).
    try:
        import yaml as _yaml

        _rc_path = os.path.join(artifacts_dir, 'resolved_config.yaml')
        ...
        logger.info('Resolved config saved to %s', _rc_path)
    except Exception as _exc:
        logger.warning('resolved_config.yaml write failed: %s', _exc)
