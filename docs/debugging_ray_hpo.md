...
| PyCharm server console: `Warning: wrong debugger version. ... pip install pydevd-pycharm~=<build>` — and PyPI does NOT have that build (common for EAP/snap builds) | PyPI releases lag PyCharm builds | Install the debugger egg SHIPPED WITH your PyCharm instead (an exact match). `uv` cannot install eggs — extract it into site-packages (hit 2026-08-10, verified): `SP=$(.venv/bin/python -c "import site; print(site.getsitepackages()[0])"); unzip -qo <pycharm-install>/debug-eggs/pydevd-pycharm.egg -d "$SP" -x "EGG-INFO/*"` (snap install path e.g. `/snap/pycharm-professional/<rev>/debug-eggs/`; find yours via `readlink /proc/$(pgrep -f pycharm | head -1)/exe`) |
