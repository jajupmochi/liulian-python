...
| CALF | arXiv 2403.07300 | cross-modal alignment | ⬜ TODO — needs a design decision first: CALF's feature/output/gradient ALIGNMENT LOSSES belong in the TASK layer (tasks own losses/metrics; models own only forward), so it is not a pure backbone swap like the others. Scope the dual-branch forward here + the alignment loss as a task-side add-on. |
