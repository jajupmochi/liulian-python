...
ETTh1 (per-epoch losses identical; Test MSE 0.3908 / MAE 0.4159). Notably, the
original Time-LLM paper reports \emph{fixed} per-dataset configurations (its
Appendix~B.4, Table~9) and describes no hyper-parameter search; its released code
does apply validation early stopping (patience 10) but no tuning. Our matched
30-sample HPO therefore strengthens every comparison relative to the original
protocol, and applies it \emph{equally} to all arms. Comparisons never mix
precision, search space, prompt file, or budget; such changes deploy only at tier
boundaries.
