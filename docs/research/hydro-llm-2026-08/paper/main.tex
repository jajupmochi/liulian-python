...
2026, denorm RMSE °C). Trainable parameters identical across arms; only the
frozen stack differs. Top row is the \textsf{none} reference; the three
\textsf{numeric} rows swap the backbone. Per-station Wilcoxon $p$ vs.\ each
dataset's \textsf{none} in the text.}
\label{tab:backbone}
\begin{tabular}{@{}lccc@{}}
\toprule
arm & \swiss{} & \swissb{} & \swissc{} \\
\midrule
\textsf{none} (pretrained, frozen) & 1.8658 & 1.8360 & 1.9407 \\
\midrule
\textsf{numeric}, pretrained frozen & 1.6471 & 1.7203 & 1.8861$^{*}$ \\
\textsf{numeric}, random-init frozen & \textbf{1.5241} & \textbf{1.5910} & \textbf{1.7417} \\
\textsf{numeric}, no LLM (0 blocks) & 1.5652 & 1.7188 & 1.8763 \\
\bottomrule
\multicolumn{4}{@{}p{0.9\linewidth}@{}}{\footnotesize $^{*}$inside the
${\approx}4\%$ \swissc{} restart band. The \textsf{none} cell is
backbone-invariant (all arms agree to ${\sim}1\%$ per dataset, inside the run-to-run band; deleting the stack
is n.s.); only \textsf{numeric} is shown per backbone.}
\end{tabular}
\end{table}
...
additive path, not in the language model --- and \emph{random-init frozen} is the
best backbone on all three collections, i.e.\ the pretrained weights actively tax
the additive channel. This reproduces Tan et al.~\cite{tan2024llmuseful} on
hydrological data and extends it with the identity axis they did not test.
...
close the picture. \swissb{} replicates \swiss{} exactly: pretrained{+}LoRA
wakes the text pathway (1.8253${\rightarrow}$1.6932, $-7.8\%$ vs.\ its
\textsf{none}, $p{=}1.1{\times}10^{-3}$, n=63) while random{+}LoRA text is
indistinguishable from its own \textsf{none} (1.8200 vs.\ 1.8118) --- on the
entity-rich collections (2/2) the wake-up \emph{requires} the pretrained
weights. \swissc{} adds two refinements. First, it resolves its own anomaly:
random-init~+~\emph{frozen} numeric reaches 1.7417 ($-10.3\%$,
$p{=}6.1{\times}10^{-5}$, n=15) on the very dataset where pretrained-frozen
numeric was null --- the pretrained geometry, not the data, was the blocker.
Second, on this smallest collection the text wake-up does \emph{not} require
pretraining (random{+}LoRA text 1.7548 matches pretrained{+}LoRA 1.8198),
consistent with text acting there as a distinctness token through an adaptable
backbone; we report this nuance rather than average over it, and note that
\swissc{} single-run claims sit inside a measured ${\approx}4\%$ restart band.
Finally, random-init numeric is the best cell on \emph{every} dataset
(1.5142/1.6079/1.7568) --- the pretrained weights tax the additive channel
universally.
